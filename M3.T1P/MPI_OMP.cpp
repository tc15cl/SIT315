...
int main(int argc, char *argv[]) {
  int numtasks, rank, num_node_workers, source, tag=1, wtag=2,rowTotal, rowNum, rowWorker, rowLeft;
  ...
  startClock = MPI_Wtime();
    ...
    for(int i = 1; i <= num_node_workers; i++) {
      ...
      // Send the number of rows, current row & matrices
    ...
    for(int i = 1; i <= num_node_workers; i++) {
      MPI_Recv(&rowTotal, 1, MPI_INT, i, wtag, MPI_COMM_WORLD, &Stat);
      MPI_Recv(&rowNum, 1, MPI_INT, i, wtag, MPI_COMM_WORLD, &Stat);
      MPI_Recv(&m3[rowNum][0], rowTotal * MATRIX_SIZE, MPI_INT, i, wtag, MPI_COMM_WORLD, &Stat);
    }
