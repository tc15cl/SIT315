...
//define number of threads to use with OMP
...
{
    //OMP pragma parallel + for statement
    #pragma omp parallel for default(none) shared(MATRIX_SIZE, matrix1, matrix2, matrix3)
	for (int i = 0; i < MATRIX_SIZE; i++)
	{
		for (int j = 0; j < MATRIX_SIZE; j++)
		{
...
int main()
{
	...
	cout << "OMP, " << "Threads: " << NUM_THREADS << ", " << MATRIX_SIZE << " X " << MATRIX_SIZE 
	...
	{	
		metricsFile << "OMP, " << "Threads: " << NUM_THREADS << ", " << MATRIX_SIZE << " X " << MATRIX_SIZE 
		<< " Matrix, Duration: " << duration.count() << "us" << endl; 
		metricsFile.close();
	}
